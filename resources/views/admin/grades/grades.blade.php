...
.btn.btn-sm.btn-primary {
  ...
  display: inline-flex; /* to make margin: 0 auto work */
...
<body>
    ...
    <div>
        ...
        @section('content')
            <h2 class="masterlist-header">Grades for {{ $student->FirstName }} {{ $student->LastName }}</h2>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach ($grades as $grade)
                        <tr class="table-data">
                            <td>{{ $grade->CourseDescription }}</td>
                            <td>{{ $grade->Midterm }}</td>
                            <td>{{ $grade->Final }}</td>
                            <td>{{ $grade->Final <= 3.00 ? 'Passed' : 'Failed' }}</td>
                            <td>
                                <a href="{{ route('grades.edit', $grade->GradeID) }}" class="btn btn-sm btn-primary">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
